...
LOOP = 10
ALPHA = 0.5
REFLECT_POW = 1.0 / 5  # для отображения маленьких чисел в числа побольше
...
def fix_score(_fix, distance, orig_popularity):
    ...
    # print("{}: ---> P(orig|fix)={}, fix_pop={}, orig_pop={} |--> result={}".
    #       format(_fix, ALPHA * distance, get_query_pop(),
    #              orig_popularity, ALPHA * distance * get_query_pop() / orig_popularity))
...
def generate_fix_query(st_matrix):
    ...
    min_key = [0]  # костыль с листом нужен для второго питона
    ...
    def add_query(local_score, string):
        ...
        if len(query_dict) <= QUERY_VARIANTS:
            ...
            return
        if local_score < min_key[0]:
            return
        min_key[0] = min(query_dict)
        query_dict.pop(min_key[0])
        query_dict[local_score] = string
