...
END_OF_WORD = "\n"
ERROR_WIGHT = 100
MAX_ERROR = 3  # adjust this parameter
ALPHA = 1  # adjust this parameter for bigger digits
SMOOTHING_POW = 1.0 / 10
PREFIX_PATH = ""
...
class BORtree:
    ...
    try:
        ...
        print('\033[93m' + " -- BORtree can not load json -- " + '\033[0m')
    ...
    def find_best(self, string):
        ...
        self._get_best_word()
        orig_bigram = bi_symbols(string)
        BETTA = 1
        for key, ungram_error in self.best_matches.items():
            # print("way: \"{}\" - ungram_error: {}, frequency: {}, : distance: {}, result = {}".
            #       format(key, ungram_error,
            #              (BETTA * BORtree.frequency.get_popularity(key)) ** SMOOTHING_POW,
            #              (BORtree.bigram_distance.get_weighted_distance(orig_bigram, bi_symbols(key))),
            #              (BETTA * BORtree.frequency.get_popularity(key)) ** SMOOTHING_POW / \
            #              (ungram_error + BORtree.bigram_distance.get_weighted_distance(orig_bigram, bi_symbols(key)))))

            self.best_matches[key] = (BETTA * BORtree.frequency.get_popularity(key)) ** SMOOTHING_POW / \
                                     (ungram_error + BORtree.bigram_distance.get_weighted_distance(orig_bigram,
                                                                                                   bi_symbols(key)))
            # print("way: \"{}\" - frequency: {} : distance: {}, result = {}".
            #       format(key,  BORtree.frequency.get_popularity(key),
            #              (ALPHA *BORtree.bigram_distance.get_weighted_distance(orig_bigram,
            #                                                            bi_symbols(key))) ** SMOOTHING_POW,
            #              (BORtree.frequency.get_popularity(key)) / \
            #              (ALPHA *BORtree.bigram_distance.get_weighted_distance(orig_bigram,
            #                                                            bi_symbols(key))) ** SMOOTHING_POW))
            #
            # self.best_matches[key] = (BORtree.frequency.get_popularity(key)) / \
            #                          (ALPHA *BORtree.bigram_distance.get_weighted_distance(orig_bigram,
            #                                                                        bi_symbols(key))) ** SMOOTHING_POW
    ...
    def _get_best_word(self, error=0, pointer=-1, node=0, small_error=0, big_error=0):
        ...
        if word and (len(self.str) - 1 == pointer):
            self._add_match(word, error + ERROR_WIGHT * (small_error + big_error))
            # print("(local) \"" + word + "\"" + " --:-- error: " + str(ERROR_WIGHT * (small_error + big_error)))
            return
        ...
        for char, new_node in branches.items():
            self._get_best_word(
                ...
                pointer=pointer,
                node=new_node,
                small_error=small_error + (not (orig_char == char)),
                big_error=big_error)

            if big_error < 1:  # пусть добавление или удаление буквы может быть только 1 раз
                # вставка
                self._get_best_word(
        ...
        if big_error < 1:  # пусть добавление или удаление буквы может быть только 1 раз
            ...
            # удаление последнего символа в слове
                ...
                self._add_match(word, error + ERROR_WIGHT * (small_error + big_error + 1))
                # print("(local) \"" + word + "\"" + " --:-- error: " + str(ERROR_WIGHT * (small_error + big_error + 1)))
    ...
    def _get_error(orig, fix):
        ...
        #                                                  orig, fix))
        return min(1 / BORtree.ungram_distance.get_popularity(orig, fix), 2000)  # костыль
...
if __name__ == "__main__":
    ...
    for num, i in enumerate(t.find_best(u"кощка")):
